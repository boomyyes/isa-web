...
export default function Globe({
    ...
    useEffect(() => {
        ...
        // Entrance state. The globe mounts lazily and its land data loads async,
        // so instead of snapping in when ready it fades + scales up — the
        // animation masks the load rather than exposing it as a pop.
        ...
        const loadWorldData = async () => {
            try {
                setIsLoading(true);
                // 110m land: ~10x smaller/simpler than 50m — far cheaper to
                // download, parse, and rasterize, and plenty of detail for a
                // small dotted globe. This is the main startup-cost lever.
                ...
                // Landmask resolution for the dot sampling. Halved from
                // 2048x1024 → the getImageData read (a fixed per-mount cost) is
                // now 1/4 the size, and this is ample for the dot spacing.
                ...
                // Flip visibility first, then kick the transition on the next
                // frame — changing both in one go would skip the animation.
